...
df_all = read.table("ctype_BAL_integrated_v3.txt",sep="\t",header=1, row.names = 1)

# get a new column called cell group
df_all$cell.group = mapvalues(df_all$Cell.class, from = c("MoAM1","MoAM2","MoAM3","MoAM4","MoAM5",
                                                          "TRAM1","TRAM2","TRAM3",
...
# create output table
write.table(df_output, "../publication/BAL_cellclass_DA_pval_adjusted.txt", sep = "\t")
...
for (dataset in unique(df_all$Dataset)) 
{
  ...
  {
    print(celltype)
    output = pval_generator(df_dataset, celltype = celltype, celltype_col = "cell.group")
    compRecoder = paste0(dataset, ", ", celltype, ", ", output[[1]])
    pvalRecoder = output[[2]]
    compRecoder_all = append(compRecoder_all, compRecoder)
    pvalRecoder_all = append(pvalRecoder_all, pvalRecoder)
...
# create output table
write.table(df_output, "BAL_cellgroup_DA_pval_adjusted.txt", sep = "\t")
